...
sudo ovs-vsctl set-controller br-int tcp:192.168.56.20:6653
sudo ovs-vsctl set-manager tcp:192.168.56.20:6640

### VM1
sudo ip link add vm1-eth0 type veth peer name vm1-br
sudo ip link set dev vm1-eth0 address f6:00:00:ff:01:02
sudo ip link set dev vm1-br address f6:00:00:0f:11:02
...
sudo ip netns exec vm1 ip addr add dev vm1-eth0  192.168.2.1/24
...
sudo ovs-vsctl add-port br-int vm1-br

### VM2
sudo ip link add vm2-eth0 type veth peer name vm2-br
sudo ip link set dev vm2-eth0 address f6:00:00:ff:01:12
sudo ip link set dev vm2-br address f6:00:00:0f:11:52
...
sudo ip netns exec vm2 ip addr add dev vm2-eth0  192.168.2.2/24
...
sudo ovs-vsctl add-port br-int vm2-br

sleep 5
sudo ip netns exec vm1 ping -c4 192.168.2.2
